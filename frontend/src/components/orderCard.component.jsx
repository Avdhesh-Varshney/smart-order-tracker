...
import { statusEmojis } from "../pages/home.page";
...
const OrderCard = ({ order, orderNum, customer }) => {
    ...
    return (
        <Link
            to={`/order/${id}`}
            className="w-[300px] h-auto rounded-2xl p-6 border border-gray-300 shadow-md hover:shadow-lg transition-shadow duration-300 cursor-pointer bg-white"
        >
...

            {
                role && customer ? (
                    ...
                    <p className="text-gray-600 text-sm mt-2 line-clamp-2">
                        {des ? des : "No description provided."}
                    </p>
                )
            }
        </Link>
